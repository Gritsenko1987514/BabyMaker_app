...
//
#define COLLISIONWIDTH 5 //distance between moving cell and collision cell
#define PAGEMOVEMARGIN 70
#define CELL_BOUNDS CGRectMake(0, 0, 78, 86);
...
-(void) CellRearrange:(NSInteger) moveIndex with:(NSInteger)targetIndex
{
    NSLog(@"here the index is %d, %d",moveIndex, targetIndex);
...
//    CGRect cellBounds = CGRectMake(0, 0, 49, 49);
    CGRect cellBounds = CELL_BOUNDS;
    
    // this is to fix the scroll view issue
//    CGSize contentSize = CGSizeMake(self.numberOfPages * gridBounds.size.width , gridBounds.size.height);
//    CGSize contentSize = CGSizeMake(gridBounds.size.width ,  gridBounds.size.height);
...
-(NSInteger) CellCollisionDetection:(UzysGridViewCell *) cell
{
    ...
    for(int i=0;i<[_cellInfo count];i++)
    {
        ...
        {
           if(CGRectContainsPoint(coll.frame, cell.center))  //collision detection
            {
...
//                if(distance < COLLISIONWIDTH)
    ...
    [collisionCells release];
     NSLog(@"collision called CellRearrange");
...
- (void) LoadTotalView {
    ...
    if(self.dataSource && self.numberOfRows > 0 && self.numberOfColumns > 0)
    {
        ...
        CGSize contentSize = CGSizeMake(200, self.numberOfRows * 118);
        ...
        for(NSUInteger i = 0 ; i< [self.dataSource numberOfCellsInGridView:self];i++)
        {
            ...
            else
            {
                ...
                contractFrame = CGRectMake((NSUInteger)origin.x, (NSUInteger)origin.y, (NSUInteger)cellBounds.size.width, (NSUInteger)cellBounds.size.height);
                CGRect frame = CGRectInset(contractFrame, self.cellMargin, self.cellMargin);
                cell.frame = frame;
            }
        ...
        if(MaxScrollwidth - _curcell.center.x < PAGEMOVEMARGIN)
        {
            if(self.numberOfPages-1 > self.currentPageIndex)
            {
                ...
                if(self.numberOfPages-1 == self.currentPageIndex)
                {
                    ...
                    NSLog(@"movePagesTimer called CellRearrange");
...
{
    self.editable =YES;
    [self startWiggling:cell];
    
}
- (void)startWiggling : (UzysGridViewCell *)cell{
    
    for(NSUInteger i = 0 ; i < [[_scrollView subviews] count] ;i++)
    {
        UzysGridViewCell *cell = (UzysGridViewCell *)[[_scrollView subviews] objectAtIndex:i];
    ...
    [cell.layer addAnimation:anim forKey:@"wiggleRotation"];
    ...
    anim.repeatCount = HUGE_VALF;
    anim.additive = YES;
    [cell.layer addAnimation:anim forKey:@"wiggleTranslationY"];
    }
}
