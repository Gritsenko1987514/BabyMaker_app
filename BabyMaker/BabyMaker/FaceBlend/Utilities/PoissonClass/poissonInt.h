        ...
        for(int j=0;j<w;j++)
            for(int c=0;c<channel;++c)
            {
                if(i+1 == h){
                    ...
                    (float)CV_IMAGE_ELEM(img,uchar,(i),j*channel+c) - (float)CV_IMAGE_ELEM(img,uchar,i,j*channel+c);
                } else{
                    CV_IMAGE_ELEM(gy,float,i,j*channel+c) =
...
IplImage *poisson_blend(IplImage *I, IplImage *mask, int posx, int posy ,BOOL isMouth)
{
    //Akhildas
//	unsigned long int idx;
...
//	int channel1 = mask->nChannels;
    ...
    int maxH1 = floorf(h1*0.6)+posx;
    int weight = -1;
    int factor = (w1/2)/((posx+h1)-maxH1);
    
    float maxH2 = ceilf((h1*0.2)+posx);
    float weight2 = (posx+h1)-maxH2;
    float factor2 = (((w1/2)-((w1/2)*0.8))/weight2);
    debugLog(@"factor%d",factor);
    ...
    for(int i=posx, ii =0;i<posx+h1;i++,ii++) {
		...
		for(int j=0,jj=posy;j<w1;j++,jj++) {
            ...
            if(i<maxH1) {
                ...
                if((abs((posy+(w1/2))-jj))<(((0+(w1/2))-(value)))) {
                    for(int c=0;c<channel;++c)
                    {
                ...
                if(i>maxH2) {
                    ...
                    if((abs((posy+(w1/2))-jj))<(((0+(w1/2)+0)-(value)))) {
                        for(int c=0;c<channel;++c)
                        {
