...
- (NSString *)message
{
    return @"Reviews are very important to little app makers like us. If you like Baby Maker, could you take a second to rate us in the App Store?";
    ...
    if (!message)
    {
        ...
        message = @"Reviews are very important to little app makers like us. If you like Baby Maker, could you take a second to rate us in the App Store?";
    }
...
{
    int count = [[NSUserDefaults standardUserDefaults] integerForKey:@"iRateTimeCount"];
    if (count >= 100) count = 0;
    if (count == 0) count = 1;
    [[NSUserDefaults standardUserDefaults] setInteger:(count + 1) forKey:@"iRateTimeCount"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    if (count % 5 > 0)
        return;
    
    NSDate *remindDate = [self lastReminded];
    if (remindDate != nil)
    {
        NSTimeInterval time = [remindDate timeIntervalSinceDate:[NSDate date]];
        int hours = ((abs(time) / (60.0 * 60.0)) + 0.5);
        if (hours < 24 * 2)
            return;
    }
